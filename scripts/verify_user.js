...
async function verifyEmail(email) {
  ...
  console.log("1. Profiles table query by email:", profileByEmail);
  ...
  if (authUser) {
    // 3. Check profiles table by user ID
    const { data: profileById, error: err3 } = await supabaseAdmin
      .from("profiles")
      .select("*")
      .eq("id", authUser.id);
    console.log("3. Profiles table query by user ID:", profileById);
  }
}

async function run() {
  await verifyEmail("rjhustles@gmail.com");
  await verifyEmail("detourdesignllc@gmail.com");
  await verifyEmail("philip@philipanders.com");
}
